...
> This is a lightweight, vanilla-only re-implementation focused on the gameplay tuning that the original mod provided. It does **not** ship the original mod's UI overhaul (saved designs panel, hotkey-driven multi-craft, custom Gauntlet prefabs). If you want those, install the original mod from Nexus alongside its `Bannerlord.Harmony` / `Bannerlord.ButterLib` / `Bannerlord.MBOptionScreen` dependencies.
...
All three values are read at game launch from a plain XML file and clamped to safe ranges.

## How it works

Uses [Harmony](https://github.com/pardeike/Harmony) runtime patching against `TaleWorlds.CampaignSystem.GameComponents.DefaultSmithingModel`:

- `GetEnergyCostForRefining` — prefix that returns `0` when *Unlimited Crafting Stamina* is on.
- `GetEnergyCostForSmithing` — prefix that returns `0` when *Unlimited Crafting Stamina* is on.
- `GetEnergyCostForSmelting` — prefix that returns `0` when *Unlimited Crafting Stamina* is on.
- `GetSmithingStaminaIncreasePerHour` — postfix that multiplies the original recovery rate by the configured in-town or wilderness multiplier (selected based on whether the hero's party currently sits in a settlement).

No game files are modified. Patches are applied on load and reverted on unload.

**Mod conflicts:** Any other mod patching `DefaultSmithingModel` (including the original Better Smithing Continued from Nexus) may conflict — pick one.
